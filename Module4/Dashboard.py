...
if option == "Hotel Booking Insights":
    ...
    # Average Length of Stay with Time Granularity Selection
    st.subheader("🛏️ Average Length of Stay")

    time_granularity = st.selectbox("Select Time Granularity", ["Weekly", "Monthly", "Yearly"])

    if time_granularity == "Weekly":
        avg_stay = filtered_booking_df.groupby("week")["length_of_stay"].mean().reset_index()
        x_column = "week"
        title = "Weekly Average Length of Stay"

    elif time_granularity == "Monthly":
        filtered_booking_df["month"] = filtered_booking_df["check_in_date"].dt.strftime('%Y-%m')
        avg_stay = filtered_booking_df.groupby("month")["length_of_stay"].mean().reset_index()
        x_column = "month"
        title = "Monthly Average Length of Stay"

    elif time_granularity == "Yearly":
        filtered_booking_df["year"] = filtered_booking_df["check_in_date"].dt.year
        avg_stay = filtered_booking_df.groupby("year")["length_of_stay"].mean().reset_index()
        x_column = "year"
        title = "Yearly Average Length of Stay"

    fig_stay = px.line(avg_stay, x=x_column, y="length_of_stay", title=title)
